...
//CLASS properties
    ...
    */
    ...
    protected $userType;      //String: usertype is either ADMIN or MEMBER
...
//CLASS methods	
    ...
    public function login($username, $password) {
        ...
        $memberTable= new MemberTable($this->db);
 
        if($adminTable->validate_login($username, $password, $this->encryptPW)){  //check if the login details match an ADMIN       
                //query the table for that specific user
                $rs=$adminTable->getRecordByID($username);
                $row=$rs->fetch_assoc(); //get the users record from the query result  
                                
                //then set the session array property values
                $this->session->setUserID($username);
            ...
            elseif($memberTable->validate_login($username, $password, $this->encryptPW)) { //check if the login details match a CUSTOMER
                //query the table for that specific user
                $rs=$memberTable->getRecordByID($username);
                $row=$rs->fetch_assoc(); //get the users record from the query result                 
