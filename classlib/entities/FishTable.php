...
class FishTable extends TableEntity {
    ...
    function __construct($databaseConnection){
        parent::__construct($databaseConnection,'fish');
    }
    ...
    public function getFullRecordByID($userID){
        $sql="SELECT name, catchLocation, sellPrice FROM fish WHERE name='$userID'";
    ...
    public function getRecordByName($name){
        $sql="SELECT name, catchLocation FROM fish WHERE name='$name'";
        if($rs=$this->db->query($sql)){
            if ($rs->num_rows===1){return $rs;}else{return false;}//only one record should be returned
        }
        else{
