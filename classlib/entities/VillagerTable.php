...
class VillagerTable extends TableEntity {
    ...
    {
        $sql = 'SELECT name FROM villagers';
    ...
    public function getFullRecordByID($name){
        $sql="SELECT * FROM villagers WHERE name='$name'";
        if($rs=$this->db->query($sql)){
    ...
    public function getRecordByName($name){
        $sql="SELECT name, personality, gender FROM villagers WHERE name='$name'";
        if($rs=$this->db->query($sql)){
            if ($rs->num_rows===1){return $rs;}else{return false;}//only one record should be returned
        }
        else{
    ...
    public function deleteRecordbyID($userID){
        ...
        if($this->getRecordByID($name)){ //confirm the record exists before deletig
            $sql = "DELETE FROM villagers WHERE name='$name'";
