...
//CLASS methods
	...
	{   
        ...
        public function setPanelHead_1(){//set the panel 1 heading 
            if ($this->pageID == 'registerMember')
                $this->panelHead_1='<h3>Become A Member!</h3>';
            
            elseif ($this->pageID == 'registerAdmin')
                $this->panelHead_1='<h3>Add An Admin To The Team!</h3>';
        ...
        public function setPanelContent_1(){//set the panel 1 content
            if ($this->pageID == 'registerMember')
            {
                $this->panelContent_1 = file_get_contents('forms/form_register_member.html');  //this reads an external form file into the string 
            }
            
            elseif ($this->pageID == 'registerAdmin')
            {
                $this->panelContent_1 = file_get_contents('forms/form_register_admin.html');
            }
        ...
        public function setPanelContent_2(){//set the panel 2 content  
            if ($this->pageID == 'registerAdmin')
            {
                if(isset($this->postArray['btnRegister']))
                {  //check that the login button is pressed
                    $data = new AdminTable($this->db);

                    if($data->addRecord($this->postArray, true)){
                        $this->panelContent_2 = 'New admin registered';
                    }
                    else{
                        $this->panelContent_2 = 'Registration NOT successful';
                    }
                }
                else{
                    $this->panelContent_2='Please fill out the details to add a new admin to the team!';
                    }
            }
            elseif ($this->pageID == 'registerMember')
            {
                if(isset($this->postArray['btnRegister']))
                {  //check that the login button is pressed
                    $data = new MemberTable($this->db);

                    if($data->addRecord($this->postArray, true)){
                        $this->panelContent_2 = 'New member registered';
                    }
                    else{
                        $this->panelContent_2 = 'Registration NOT successful';
                    }
                    ...
                    $this->panelContent_2='Please fill out the details in the form to become a member!';
                    }
            }
